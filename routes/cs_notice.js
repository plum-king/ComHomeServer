...
const fileFields = upload.fields([
  ...
  router.post("/post", fileFields, async (req, res) => {
    ...
    const params=[notice_id, req.user.id, title, content, date, date, 0, notice_img, count >0 ? 1 :0 ]; //count >0 ? 1 :0 -> 첨부파일 여부확인

    try {
        const data = await pool.query(sql,params);
        res.write(`<script type="text/javascript">alert('CS notice post Success !!')</script>`);
    ...
    for(let i=0;i<count;i++){
      try {
        const data = await pool.query(`INSERT INTO file_cs(no, file_infoN, file_originN) VALUES(?,?,?)`,[notice_id, req.files.file[i].path, req.files.file[i].originalname]);
      } catch (err) {
        console.error(err);
      }
    }
