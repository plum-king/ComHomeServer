...
router.post("/", async (req, res) => {
  ...
  const my_secret_checkbox = post.my_secret_checkbox;
  const my_anon_checkbox = post.my_anon_checkbox;

  try {
    const data = await pool.query(
      `INSERT INTO edu_contest_comment(content, iduser, edu_contest_no) VALUES(?, ?, ?)`,
