...
router.post("/update", fileFields, async (req, res) => {
  ...
  const sql1 =
    "UPDATE recruit_intern SET title=?, content=?, edited_date=?, img=? WHERE no=?";
  const params1 = [title, cont, date, notice_img, notice_id];

  //수정할때 이미지 추가 안한경우에는 update문에서 img 속성은 뺴야함
  const sql2 =
    "UPDATE recruit_intern SET title=?, content=?, edited_date=? WHERE no=?";
...
router.post("/delete", async (req, res) => {
  ...
  const file_status = await pool.query(
    `SELECT file_status FROM recruit_intern WHERE no=${notice_id}`
  );
  ...
  try {
    const data = await pool.query(`DELETE FROM recruit_intern WHERE no=?`, [
      notice_id,
    ]);
    status = 200;
  } catch (err) {
